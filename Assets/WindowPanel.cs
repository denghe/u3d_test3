...
public class WindowPanel : MonoBehaviour, IDragHandler, IPointerDownHandler
{
    ...
    void SnapEachOther()
    {
        foreach (var w in allWindows)
        {
            ...
            {
                var diffMin = w.trans.position - (Vector3)(w.trans.rect.position + trans.rect.position) -
                              trans.position;
                var diffMax = w.trans.position + (Vector3)(w.trans.rect.position + trans.rect.position) -
                              trans.position;
